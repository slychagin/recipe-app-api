...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_update_user_returns_error(self):
        """Test changin the recipe user results in an error"""
    ...
    def test_delete_other_users_recipe_error(self):
        """Test trying ti delete another users recipe gives error"""
        new_user = create_user(email='user2@example.com', password='test123')
    ...
    def test_create_recipe_with_existing_ingredent(self):
        ...
        payload = {
            ...
            'time_minutes': 50,
            'price': Decimal('2.50'),
            'ingredients': [{'name': 'Lemon'}, {'name': 'Pepper'}]
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_create_ingredient_on_update(self):
        ...
        payload = {'ingredients': [{'name': 'Cardamon'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = Ingredient.objects.get(
            user=self.user, name='Cardamon')
